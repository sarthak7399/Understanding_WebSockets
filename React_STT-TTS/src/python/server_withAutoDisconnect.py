...
# Define wait_for_message function outside WebSocket endpoint function
async def wait_for_message(websocket):
    # Wait for message indicating audio playback completion
    ...
    try:
        while True:
            ...
            if message:
                ...
                # Start timer
                timer_task = asyncio.create_task(wait_for_message(websocket))

                try:
                    # Wait for message indicating audio playback completion or timeout (15 seconds)
                    await asyncio.wait_for(timer_task, timeout=10+(extratime/16))
                except asyncio.TimeoutError:
                    # Handle timeout
                    print("No message received within 10 seconds. Closing connection.")
